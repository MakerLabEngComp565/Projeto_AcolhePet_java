        ...
        do {
            ...
            switch (opcaoUsuario) {
                ...
                case 3:
                    ...
                    // Retorna o animal para Disponível
                    cancelarAdocao.getAnimal().setStatus("Disponível");
                        ...
                        switch (opcaoAdmin) {
                            ...
                            case 2:
                                ...
                                if (animalController.listarAnimais().isEmpty()) {
                                    ...
                                    for (Animal a : animalController.listarAnimais()) {
                                        String racaExibida = a.getRaca().contains("Sem Ra") ? "Sem Raça Definida" : a.getRaca();
                                        System.out.println(ANSI_YELLOW + "ID: " + a.getId() + " | Nome: " + a.getNome() + " | Tipo: " + a.getTipo() + " | Raça: " + racaExibida + " | Idade: " + a.getIdade() + " | Vacinação: " + a.getStatusVacinacao() + " | Status: " + a.getStatus() + ANSI_RESET);
                                    }
                                }
                                break;
                            ...
                            case 4:
                                ...
                                // 🌟 PROTEÇÃO ADICIONADA: try-catch para capturar qualquer erro de digitação e retornar ao menu admin
                                try {
                                    ...
                                    if (animalExcluir != null) {
                                        ...
                                        System.out.println(ANSI_GREEN + "Animal removed com sucesso!" + ANSI_RESET);
                                    } else {
                            ...
                            case 5:
                                ...
                                // 🌟 EXIBIÇÃO DA LISTA DE ADOÇÕES ANTES DAS PERGUNTAS
                                System.out.println(ANSI_YELLOW + "Lista de adoções cadastradas para remoção:" + ANSI_RESET);
                                for (Adocao a : adocaoController.listarAdocoes()) {
                                    String racaFormatada = a.getAnimal().getRaca().contains("Sem Ra") ? "Sem Raça Definida" : a.getAnimal().getRaca();
                                    System.out.println(ANSI_YELLOW + "ID Animal: " + a.getAnimal().getId() + " | Nome Animal: " + a.getAnimal().getNome() + " | Adotante: " + a.getPessoa().getNome() + " | E-mail: " + a.getPessoa().getEmail() + " | Telefone: " + a.getPessoa().getTelefone() + " | Tipo: " + a.getAnimal().getTipo() + " | Raça: " + racaFormatada + " | Idade: " + a.getAnimal().getIdade() + " | Vacinação: " + a.getAnimal().getStatusVacinacao() + ANSI_RESET);
                                }

                                // Captura os dados do animal e do adotante associados à adoção dentro de uma estrutura segura try-catch
                                try {
                                    System.out.print("\nQual é o ID do Seu Animal?: ");
                                    int remAdocaoId = Integer.parseInt(sc.nextLine());

                                    System.out.print("Digite o nome do animal: ");
                                    String remAdocaoNome = sc.nextLine();
                                    System.out.print("Digite o tipo do animal: ");
                                    String remAdocaoTipo = sc.nextLine();
                                    System.out.print("Qual Seria a Raça do Animal?: ");
                                    String remAdocaoRaca = sc.nextLine();
                                    System.out.print("Quantos anos o animal tem: ");
                                    int remAdocaoIdade = Integer.parseInt(sc.nextLine());
                                    System.out.print("Qual é o Status da Vacinação do animal? ");
                                    String remAdocaoVacina = sc.nextLine();

                                    System.out.print("Digite o nome do adotante: ");
                                    String remAdocaoNomePessoa = sc.nextLine();
                                    System.out.print("Digite o email do adotante: ");
                                    String remAdocaoEmailPessoa = sc.nextLine();
                                    System.out.print("Digite o telefone do adotante: ");
                                    String remAdocaoTelefonePessoa = sc.nextLine();

                                    Adocao adocaoRemover = null;
                                    for (Adocao a : adocaoController.listarAdocoes()) {
                                        Animal anim = a.getAnimal();
                                        Pessoa pess = a.getPessoa();
                                        if (anim.getId() == remAdocaoId
                                            && anim.getNome().equalsIgnoreCase(remAdocaoNome)
                                            && anim.getTipo().equalsIgnoreCase(remAdocaoTipo)
                                            && (anim.getRaca().equalsIgnoreCase(remAdocaoRaca) || anim.getRaca().contains("Sem Ra"))
                                            && anim.getIdade() == remAdocaoIdade
                                            && anim.getStatusVacinacao().equalsIgnoreCase(remAdocaoVacina)
                                            && pess.getNome().equalsIgnoreCase(remAdocaoNomePessoa)
                                            && pess.getEmail().equalsIgnoreCase(remAdocaoEmailPessoa)
                                            && pess.getTelefone().equalsIgnoreCase(remAdocaoTelefonePessoa)) {
                                            adocaoRemover = a;
                                            break;
                                        }
                                    }

                                    if (adocaoRemover != null) {
                                        // Retorna o status do animal para Disponível
                                        adocaoRemover.getAnimal().setStatus("Disponível");
